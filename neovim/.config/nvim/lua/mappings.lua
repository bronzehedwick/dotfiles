...
vim.keymap.set('n', '<F4>', ':let _s=@/<Bar>:%s/\\s\\+$//e<Bar>:let @/=_s<Bar><CR>', { silent = true })
-- }}}
...
vim.keymap.set('n', '<F3>', function()
    ...
    local branch = io.popen('git -C ' .. dir .. ' cb')
    if branch then
        local output = branch:read()
        branch:close()
        if output then
            print(output)
        else
...
vim.keymap.set('n', '<leader>c', ':tabedit ~/.dotfiles/neovim/.config/nvim/init.lua<CR>:tcd ~/.dotfiles/neovim/.config/nvim<CR>')
...
vim.keymap.set({'n', 't'}, '<C-j>', function()
    ...
    if vim.fn.bufwinnr(Term_buf_id) == -1 then
        vim.cmd('botright sbuffer' .. Term_buf_id)
        Term_win_id = vim.fn.bufwinid('%')
